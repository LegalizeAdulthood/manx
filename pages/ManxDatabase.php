...
class ManxDatabase implements IManxDatabase
{
    ...
    public function addSiteUnknownPath($siteName, $path)
    {
        ...
        $rows = $this->execute("SELECT `id` FROM `site_unknown_dir` WHERE `site_id`=? AND `directory`=?", array($siteId, $directory));
        if (count($rows) == 0)
        {
            $rows = $this->execute("INSERT INTO `site_unknown_dir`(`site_id`,`directory`) VALUES (?,?)", array($siteId, $directory));
            $rows = $this->execute("SELECT LAST_INSERT_ID() AS `id`;", array());
        }
        $directoryId = $rows[0]['id'];
        $this->execute("INSERT INTO `site_unknown`(`site_id`,`site_unknown_dir_id`,`file_name`) VALUES (?,?,?)",
            array($siteId, $directoryId, $fileName));
    }
    ...
    public function ignoreSitePath($siteName, $path)
    {
        ...
        $directoryId = $rows[0]['id'];
        $this->execute("UPDATE `site_unknown` SET `ignored`=1 WHERE `site_id`=? AND `site_unknown_dir_id`=? AND `file_name`=?",
            array($siteId, $directoryId, $fileName));
    }
    ...
    public function getSiteUnknownPathsOrderedByPath($siteName, $start, $ascending)
    {
        ...
        return $this->execute("SELECT CONCAT(`directory`, '/', `file_name`) AS `path`,`su`.`id` "
            ...
            . "AND `site_unknown_directory_id`=`sud`.`id` "
            . "AND `ignored`=0 "
            . "ORDER BY `path` $order LIMIT $start, 10",
            array($this->siteIdForName($siteName)));
    }
    ...
    {
        return $this->execute("SELECT `id`,CONCAT(`directory`, '/', `file_name`) AS `path` "
                . "FROM `site_unknown` `su`, `site_unknown_dir` `sud` "
                . "WHERE `site_id`=? "
                . "AND `site_unknown_directory_id`=`sud`.`id` "
                . "ORDER BY `id`",
    ...
    {
        $siteId = $this->siteIdForName($siteName);
        return $this->execute("SELECT CONCAT(`sud`.`directory`, '/', `file_name`) AS `path`, su.id AS `path_id`, copy.url, copy.copy_id, copy.md5 "
